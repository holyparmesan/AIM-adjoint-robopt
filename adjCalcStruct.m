function adj_maps = adjCalc(E12, E21, index, mode2, distanceScale, varargin)
% The first three inputs are structs of 2D field/index monitors, to be taken directly from Lumerical's varFDTD.
% The fourth input is a struct taken from a mode expansion monitor.
% The fifth input is a boolean, describing whether to scale the adjoint sensitivity by the spacing between contour points.
...
for i = 1:nargin-5
    ...
    if (distanceScale)
        scaling = vecnorm(deltas.').' * (mode2.f./squeeze(mode2.N)).' * (pi * 8.8142e-12j/ 4); 
    else
        scaling = ones(N,1) * (mode2.f./squeeze(mode2.N)).' * (pi * 8.8142e-12j/ 4);
    end
